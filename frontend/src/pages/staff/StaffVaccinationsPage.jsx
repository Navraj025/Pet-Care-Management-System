  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
          <h2 className="text-xl sm:text-2xl font-extrabold text-slate-900 dark:text-slate-100">Vaccination Management</h2>
          <p className="text-xs text-slate-500 dark:text-slate-400">Record vaccine immunizations, batch numbers, and booster due dates</p>
        </div>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {vaccinations.map((vac) => (
                  <tr key={vac.id} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/60 transition-colors">
                    <td className="p-4 font-bold text-slate-900 dark:text-slate-100">{vac.pet?.name || 'Patient'}</td>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-4 transition-colors">
            <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100">Administer Vaccination</h3>
            <form onSubmit={handleAdministerVaccine} className="space-y-3">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Vaccine Name *</label>
                <input
                  type="text"
                  required
                  value={vaccineName}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Date Administered</label>
                  <input
                    type="date"
                    required
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Next Due Date</label>
                  <input
                    type="date"
                    value={nextDueDate}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Batch / Serial Number</label>
                <input
                  type="text"
                  value={batchNumber}
