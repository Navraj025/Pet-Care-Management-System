...
const StaffAppointmentsPage = () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-extrabold text-slate-900 dark:text-slate-100">Assigned Appointments</h2>
        <p className="text-xs text-slate-500 dark:text-slate-400">Manage patient appointments, check-ins, and consultation progress</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-slate-900 p-12 rounded-3xl border border-slate-200 dark:border-slate-800 text-center space-y-3 transition-colors">
          ...
          <h3 className="font-bold text-slate-800 dark:text-slate-200 text-base">No Assigned Appointments</h3>
          <p className="text-xs text-slate-500 dark:text-slate-400">You currently have no patient appointments assigned.</p>
        </div>
      ) : (
            ...
            <table className="w-full text-left text-xs min-w-[750px]">
              <thead className="bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-bold border-b border-slate-200 dark:border-slate-700">
                <tr>
                  ...
                  <th className="p-4">Booked Services</th>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {appointments.map((appt) => {
                  const servicesList = appt.appointment_services && appt.appointment_services.length > 0
                    ? appt.appointment_services.map((as) => as.service?.name || 'Service')
                    : appt.service
                    ? [appt.service.name]
                    : ['Veterinary Care'];

                  return (
                    <tr key={appt.id} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/60 transition-colors">
                      <td className="p-4 font-bold text-slate-900 dark:text-slate-100">
                        {appt.appointment_date} <br />
                        <span className="text-[11px] text-teal-700 dark:text-teal-400 font-normal">{appt.start_time} - {appt.end_time}</span>
                      </td>
                      <td className="p-4 font-bold text-slate-800 dark:text-slate-200">
                        {appt.pet?.name} <span className="text-[11px] font-normal text-slate-500 dark:text-slate-400">({appt.pet?.species})</span>
                      </td>
                      ...
                      <td className="p-4 text-right">
                        <button
                          ...
                          className="px-3 py-1.5 font-bold text-xs bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 rounded-xl transition-colors inline-flex items-center space-x-1"
                        >
      ...
      {selectedAppt && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-4 transition-colors">
            <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100">Update Appointment Status</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Patient: <strong className="text-slate-800 dark:text-slate-200">{selectedAppt.pet?.name}</strong>
            </p>

            <form onSubmit={handleUpdateStatus} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Select New Status</label>
                <select
                  value={newStatus}
