    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {records.map((rec) => (
            <div key={rec.id} className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xs space-y-3 text-xs transition-colors">
              <div className="flex justify-between items-start border-b border-slate-100 dark:border-slate-800 pb-3">
                <div>
                  <h3 className="font-bold text-slate-900 dark:text-slate-100 text-base">Patient: {rec.pet?.name}</h3>
                  <p className="text-slate-400">Owner: {rec.pet?.customer?.user?.full_name}</p>
                </div>
                <div className="text-right">
                  <span className="font-bold text-teal-700 dark:text-teal-300 text-sm block">{rec.date}</span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-slate-50 dark:bg-slate-800/60 p-3.5 rounded-2xl border border-slate-100 dark:border-slate-700">
                  <strong className="text-slate-700 dark:text-slate-300 block mb-1">Diagnosis:</strong>
                  <p className="text-slate-900 dark:text-slate-100 font-semibold">{rec.diagnosis}</p>
                </div>
                <div className="bg-slate-50 dark:bg-slate-800/60 p-3.5 rounded-2xl border border-slate-100 dark:border-slate-700">
                  <strong className="text-slate-700 dark:text-slate-300 block mb-1">Treatment:</strong>
                  <p className="text-slate-800 dark:text-slate-200">{rec.treatment}</p>
                </div>
              </div>
              ...
              {rec.prescription && (
                <div className="p-3 bg-teal-50 dark:bg-teal-950/60 border border-teal-200 dark:border-teal-800 rounded-xl text-teal-900 dark:text-teal-200">
                  ...
                  <span className="text-teal-800 dark:text-teal-300 font-semibold">{rec.prescription}</span>
                </div>
              )}
            </div>
          ))}
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-4 max-h-[90vh] overflow-y-auto transition-colors">
            <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100">New Clinical Record Entry</h3>
            <form onSubmit={handleCreateRecord} className="space-y-3">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Symptoms Reported</label>
                <input
                  type="text"
                  value={symptoms}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Clinical Diagnosis *</label>
                <input
                  type="text"
                  required
                  value={diagnosis}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Treatment Administered *</label>
                <textarea
                  rows="2"
                  required
                  value={treatment}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Prescription Details</label>
                <input
                  type="text"
                  value={prescription}
