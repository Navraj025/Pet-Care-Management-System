...
const PaymentsInvoicesPage = () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-extrabold text-slate-900 dark:text-slate-100">Payments & Invoices</h2>
        <p className="text-xs text-slate-500 dark:text-slate-400">Track transaction history in INR (₹) and view printable itemized receipts</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-slate-900 p-12 rounded-3xl border border-slate-200 dark:border-slate-800 text-center space-y-3 transition-colors">
          <CreditCard className="w-12 h-12 text-slate-300 dark:text-slate-600 mx-auto" />
          <h3 className="font-bold text-slate-800 dark:text-slate-200 text-base">No Payment History</h3>
            ...
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {payments.map((pay) => {
                  ...
                  return (
                    <tr key={pay.id} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/60 transition-colors">
                      ...
                      <td className="p-4 text-slate-700 dark:text-slate-300 font-medium">{pay.payment_method.replace('_', ' ')}</td>
                      <td className="p-4 font-mono text-[11px] text-slate-500 dark:text-slate-400">{pay.transaction_id || 'Pending'}</td>
                      <td className="p-4 font-extrabold text-slate-900 dark:text-slate-100 text-sm">{formatCurrency(pay.final_amount)}</td>
                      <td className="p-4"><StatusBadge status={pay.status} /></td>
                      <td className="p-4 text-right">
                        {inv ? (
                          <Link
                            to={`/customer/invoices/${inv.id}`}
                            className="inline-flex items-center space-x-1 font-bold text-teal-600 dark:text-teal-400 hover:text-teal-800 dark:hover:text-teal-200 bg-teal-50 dark:bg-teal-950 px-3 py-1.5 rounded-xl border border-teal-200 dark:border-teal-800"
                          >
