    ...
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <div className="bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xs space-y-4 transition-colors">
        ...
        {appointments.length === 0 ? (
          <p className="text-xs text-slate-500 dark:text-slate-400 py-4">You have no completed appointments available to review yet.</p>
        ) : (
          <form onSubmit={handleSubmitReview} className="space-y-4">
            <div>
              <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Select Completed Visit</label>
              <select
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Your Comments & Feedback</label>
              <textarea
                rows="3"
                value={comment}
      ...
      <div className="space-y-4">
        <h3 className="font-bold text-lg text-slate-900 dark:text-slate-100">Recent Customer Reviews</h3>
        <div className="space-y-3">
          {reviews.map((rev) => (
            <div key={rev.id} className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs space-y-2 text-xs transition-colors">
              <div className="flex justify-between items-center">
                ...
                <span className="text-slate-400 dark:text-slate-400">{rev.created_at?.slice(0, 10)}</span>
              </div>
              <p className="text-slate-800 dark:text-slate-200 font-semibold">{rev.comment || "Great service!"}</p>
              <p className="text-[11px] text-slate-400 dark:text-slate-400">Service: {rev.service?.name}</p>
            </div>
          ))}
        </div>
      </div>
